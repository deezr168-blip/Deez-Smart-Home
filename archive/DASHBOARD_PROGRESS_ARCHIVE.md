# Dashboard progress archive

Full batch narratives for completed work, moved out of `DASHBOARD_PROGRESS.md`
to keep the recurring read cheap. **Nothing here is active work.** Every batch
below is closed; its current verification state lives in
`DASHBOARD_ISSUES.md`, and `DASHBOARD_PROGRESS.md` keeps a one-line record of
each with that state.

Read this file only when investigating a regression or revisiting a past
design decision. Newest first.
