...
| UI-032 | **Diagnostic probe** — battery conditionals failed live twice; an unconditional probe card now sits first in the same section **[diagnostic]** | Home view, top of the alert strip, directly under the page header. Look for a **purple bug icon** card reading `UI-032 PROBE v1` | **Report exactly one of three:** (a) it shows `FD 20 · RR 21 · NW 28 · SW 100 · EW unavailable · BC 100` and "3 of 6 read under 30" → entity IDs and deploy are fine, the fault is `numeric_state` evaluation; (b) it shows `FD unknown · RR unknown · …` and "0 of 6" → **the entity IDs do not resolve on this instance**, which explains both failures; (c) **no purple card appears at all** → the container is not rendering or the deploy is not at the stamped commit. Please also say whether any other alert card (e.g. "Solar producing") is visible in that same strip | `PENDING-SHA` | P2 | PENDING (attempts `a183d5f` and `0fec9bb` both returned FAIL — nothing rendered) |
