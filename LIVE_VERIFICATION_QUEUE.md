...
| UI-032 | Battery health alerts on the Active Now strip — **reworked after a live FAIL**, `condition: or` replaced with per-entity single-condition gates **[guard][中]** | With Front Door 20%, RingRing 21%, C425 North Wall 28% and East Wall unavailable, the Active Now strip on Home | **Four half-width cards**: "Front Door 20% / Battery low", "RingRing 21% / Battery low", "North Wall 28% / Battery low" (all red) and "East Wall not reporting / Battery not reporting" (orange). **Then the clearing half:** charge one above 30% and its card must disappear on its own; when none is low and none is unavailable no battery card may remain. Also confirm the `cameras` "6/6 online" chip is untouched and still says nothing about battery | `PENDING-SHA` | P2 | PENDING (previous attempt `a183d5f` returned FAIL — nothing rendered) |
