...
| CFG-003 | **Deployment / source-of-truth check — supersedes the UI-032 row.** Three pushes, including an unconditional card, produced no visible change | **(a)** Dashboard → Raw configuration editor → search `UI-032 PROBE v1`. **(b)** Same editor, search `front_door_battery`. **(c)** On `Home`, the block *directly under the page header* (above `Today`/`Rooms`, **not** the `Home Systems` grid at the bottom) — is **any** alert card there? `Solar producing` should be. **(d)** Run `/config/deploy_deez_dashboard.sh` and capture full output. **(e)** Settings → Dashboards — entry count, `url_path` values, duplicates, and which one the phone has open | **(a) absent → deployment confirmed broken; nothing since `26c3b14` has been delivered. (a) present → delivery is fine and the fault is rendering.** (c) distinguishes "probe missing" from "looked at the wrong part of the page" | `2183177` | **P1** | PENDING |
