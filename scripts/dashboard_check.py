...
def check(path):
    ...
    # to 'on' or 'off' is silently always false, so the card falls through to
    # its "not reporting" branch forever and looks like a dead sensor. This
    # shipped once on the Ray Bedroom blind chip and was caught by rendering.
    NEVER_ON_OFF = ("cover", "climate", "vacuum", "media_player")
    onoff = re.compile(
        r"states\(\s*'((?:" + "|".join(NEVER_ON_OFF) + r")\.[a-z0-9_]+)'\s*\)\s*==\s*'(?:on|off)'"
        r"|is_state\(\s*'((?:" + "|".join(NEVER_ON_OFF) + r")\.[a-z0-9_]+)'\s*,\s*'(?:on|off)'\s*\)")
    bad_onoff = [(m.group(1) or m.group(2)) for m in onoff.finditer(raw)]
    for e in bad_onoff:
        fails.append(f"{path}: {e} is compared to 'on'/'off', which its domain "
