...
| REG-012 | MED | `LIVE_VERIFICATION_QUEUE.md` (tracking, not the dashboard YAML) | Verification-queue upkeep fell behind the REG-007..011 / UI-030 batch, in two ways: (1) `691689a` (UI-030) never added a queue row for the LetPot Grow Light guard — grepping the queue for `LetPot`/`ray-bedroom`/`UI-030` returns nothing, so the human owner's checklist never surfaces this fix and it can never be marked `LIVE_VERIFIED` or reopened through the normal process; (2) the queue's own "N checks pending" footer undercounts its table: `b058006` and `d592692` added five rows (REG-007..011) without touching the footer, which stayed at "33 checks pending"; `23c0301` then bumped it to "34" (accounting only for the new `BILL-001` row) while still not accounting for REG-007..011. Actual pending-row count in the table today is 39, not the stated 34 — a gap of 5, exactly the REG-007..011 rows plus the still-absent UI-030 row. `PROJECT_STATE.md`'s own Authority table calls this file "the only place live-check instructions live," so a fix with no row here has no path to human verification. | OPEN |

**Recommended corrective action for REG-012:** add one `LIVE_VERIFICATION_QUEUE.md` row for `UI-030` (Global or Rooms section — LetPot Grow Light card, `ray-bedroom`) with commit `691689a`, and correct the footer count from "34" to the table's actual pending-row total at time of fix. No dashboard YAML change involved; in scope for the Regression Auditor or the Daily Project Coordinator, not Main or Billing.
