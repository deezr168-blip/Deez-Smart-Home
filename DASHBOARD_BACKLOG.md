...
| Battery health surfaced as a conditional Active Now alert on `home` — appears only when a verified battery is below 30% or not reporting, naming the devices; camera *connectivity* (`N/6 online` chip) left untouched and semantically separate | `UI-032` (RingRing portion now included; see `DASHBOARD_ISSUES.md`) | `a183d5f` |
