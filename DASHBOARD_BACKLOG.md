...
| Battery health surfaced as conditional Active Now alerts on `home` — one card per verified entity per situation (low / not reporting), appearing only when that entity is genuinely below 30% or unavailable; camera *connectivity* (`N/6 online` chip) left untouched and semantically separate. **Reworked after a live FAIL**: the first attempt's `condition: or` gate rendered nothing | `UI-032` (attempt 1 `a183d5f` FAIL, attempt 2 `PENDING-SHA`; see `DASHBOARD_ISSUES.md`) | `PENDING-SHA` |
